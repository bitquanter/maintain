...
from .utils import fixed_round
from .datetime_utils import to_timestamp, to_date, vec2date, to_datetime, vec2trimtime
...
class BcolzDayStore(object):
    ...
    def open_bcolz_carray(self, security, col):
        p = cfg.get_bcolz_day_path(security)
        return retry_bcolz_open(os.path.join(p, col), carray=True)
    ...
    def get_bar_by_period(self, security, start_date, end_date, fields, include_now=True):
        ...
        `security`: 标的代码，例如 'huobi/btc.usdt'。
...
def calc_coin_minute_index(security, date_index, dt, side='left', include_now=True):
    '''
    # 计算dt 在分钟数据中的索引（只对标的有效）
...
class BcolzMinuteStore(object):
    ...
    @lru_cache(None)
    def open_bcolz_table(self, security):
        p = cfg.get_bcolz_minute_path(security)
        try:
            ct = retry_bcolz_open(p)
    ...
    def get_bar_by_period(self, security, start_dt, end_dt, fields, include_now=True):
        ...
        start_idx = calc_coin_minute_index(security, ct.index, start_dt, side='right')
        if start_idx < 0:
            start_idx = 0
        end_idx = calc_coin_minute_index(security, ct.index, end_dt, include_now=include_now)
    ...
    def get_bar_by_count(self, security, end_dt, count, fields, include_now=True):
        ...
        if start_idx < 0:
            start_idx = 0
        data = {name: ct.table.cols[name][start_idx:end_idx+1]/_COL_POWERS.get(name, 1) for name in fields}
        data['date'] = ct.table.cols['date'][start_idx:end_idx+1]
        return data
    ...
    def get_bar_by_dt(self, security, somedt):
        ...
        if ct is None:
            return None
        idx = calc_coin_minute_index(security, ct.index, somedt)
