...
def open(code,end_date=None,fq=None):
    temp = get_price(code,None,end_date,'1d', ['open'], 350)['open']
    return temp
def high(code,end_date=None,fq=None):
    temp = get_price(code,None,end_date,'1d', ['high'], 350)['high']
    return temp
def low(code,end_date=None,fq=None):
    temp = get_price(code,None,end_date,'1d', ['low'], 350)['low']
    return temp
def close(code,end_date=None,fq=None):
    temp = get_price(code,None,end_date,'1d', ['close'], 350)['close']
    return temp
def vwap(code,end_date=None,fq=None):
    temp = get_price(code,None,end_date,'1d', ['avg'], 350)['avg']
    return temp
def volume(code,end_date=None,fq=None):
    temp = get_price(code,None,end_date,'1d', ['volume'], 350)['volume']
    return temp
def amount(code,end_date=None,fq=None):
    temp = get_price(code,None,end_date,'1d', ['money'], 350)['money']
    return temp
def benchmark_index_open(code,benchmark,end_date=None,fq=None):
    output=pd.DataFrame()
    temp = get_price(benchmark,None,end_date,'1d', ['open'], 350)['open']
    for coin in code:
        output[coin]=temp
    return output
def benchmark_index_close(code,benchmark,end_date=None,fq=None):
    output=pd.DataFrame()
    temp = get_price(benchmark,None,end_date,'1d', ['close'], 350)['close']
    for coin in code:
        output[coin]=temp
    return output
...
def func_sign(a):
    result = pd.DataFrame()
    for coin in a.index:
        result[coin] = np.sign(a[coin])
    return result

#*获取序列a的末位值在过去n天的顺序排位*
def func_tsrank(a,n):
    output = pd.DataFrame()
    for coin in a.columns:
        temp = [0]*(len(a)-n)
        for i in range(len(a)-n):
            temp[i] = a[coin][i:i+n].rank(pct=True)[-1]
            output[coin] = temp
...
def func_coviance(a,b,n):
    output = pd.Series()
    for coin in a.columns:
        output[coin] = float(cov(a[coin][-n:],b[coin][-n:])[0,1])
...
def func_prod(a,n):
    output = pd.Series()
    for coin in a.columns:
        output[coin] = cumprod(a[coin][-n:])[-1]
...
def func_regbeta(a,b,n):
    ...
    for coin in a.columns:
        ...
        model = linreg.fit(a[coin][-n:].reshape(n,1), b[coin][-n:].reshape(n,1))
        output[coin] = float(model.coef_)
...
def func_regresi(a,b,n):
    ...
    for coin in a.columns:
        linreg = LinearRegression()
        model = linreg.fit(a[coin][:n].reshape(n,1), b[coin][:n].reshape(n,1))
        temp = pd.DataFrame(model.predict(a[coin][:n].reshape(n,1))-b[coin][:n].reshape(n,1),columns = [coin])
        temp_s = pd.Series(temp[coin])
        output[coin] = temp_s
...
def func_wma(a,n):
    ...
    for coin in a.columns:
        temp = 0
        for i in range(n):
            temp = a[coin][i]*0.9*(i+1)+temp
        output[coin] = pd.Series(temp)
        output = pd.DataFrame(output)
    return output

#计算值(OPEN<=DELAY(OPEN,1)?0:MAX((HIGH-OPEN),(OPEN-DELAY(OPEN,1)))),其中code：标的代码列表，end_date：截止日期，默认值为None*
...
#计算值(OPEN>=DELAY(OPEN,1)?0:MAX((OPEN-LOW),(OPEN-DELAY(OPEN,1)))),其中code：标的代码列表，end_date：截止日期，默认值为None*
...
#计算值MAX(MAX(HIGH-LOW,ABS(HIGH-DELAY(CLOSE,1))),ABS(LOW-DELAY(CLOSE,1))),其中code：标的代码列表，end_date：截止日期，默认值为None
def func_tr(code,end_date=None,fq=None):
    output_f = pd.DataFrame()
    for coin in code:
        temp_1 = list(high(coin,end_date)['high']-low(coin,end_date)['low'])
        temp_hdelay = list(abs(high(coin,end_date)['high'] - close(coin,end_date).shift()['close']))
        temp_ldelay = list(abs(low(coin,end_date)['low'] - close(coin,end_date).shift()['close']))
        output = [0]*(len(temp_1)-1)
        for i in range(1,(len(temp_1)-1)):
            output[i-1] = max(max(temp_1[i],temp_hdelay[i]),temp_ldelay[i])
        output_f[coin] = pd.Series(output)
    output_f.index = open(code,end_date,fq)[1:].index
    return output_f

#计算值HIGH-DELAY(HIGH,1),其中code：标的代码列表，end_date：截止日期，默认值为None*
def func_hd(code,end_date=None,fq=None):
    output = pd.DataFrame()
    for coin in code:
        output[coin] =high(coin,end_date)['high']-high(coin,end_date).shift()['high']
    return output

#计算值DELAY(LOW,1)-LOW,其中code：标的代码列表，end_date：截止日期，默认值为None*
def func_ld(code,end_date=None,fq=None):
    output = pd.DataFrame()
    for coin in code:
        output[coin] =low(coin,end_date).shift()['low']-low(coin,end_date)['low']
    return output
...
def alpha_001(code,end_date=None,fq=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    data1 = np.log(volume(code,end_date,fq)).diff(1).rank(axis=0,pct=True)
...
def alpha_002(code,end_date=None,fq=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp = (((close(code,end_date,fq)-low(code,end_date,fq))-(high(code,end_date,fq)-close(code,end_date,fq)))/(high(code,end_date,fq)-low(code,end_date,fq))).diff(1)
    alpha = temp.iloc[-1,:]
    return alpha*-1
...
def alpha_003(code,end_date=None,fq=None):
    ...
    part3 = (close(code,end_date,fq)-np.maximum(delay1[condition3],high(code,end_date,fq)[condition3])).iloc[-6:,:]
...
def alpha_021(code,end_date=None,fq=None):
    ...
    output = pd.Series()
    for coin in code:
        A[coin]=A[coin].fillna(0)
        linreg = LinearRegression()
        model = linreg.fit(A[coin].values[-6:].reshape(6,1), B[-6:].reshape(6,1))
        output[coin] = float(model.coef_)
    alpha=output
    for coin in code:
        if alpha[coin]==0:
...
def alpha_075(code, benchmark, end_date=None, fq=None):
    '''
    公式：
...
def alpha_149(code, benchmark, end_date=None, fq=None):
    '''
    公式：
    ...
    output=pd.Series()
    for coin in code:
        temp_1=part1[coin].fillna(0)
        temp_2=part2[coin].fillna(0)
...
#公式不符合逻辑，已经补足
def alpha_181(code, benchmark, end_date=None, fq=None):
    '''
    公式：
...
def alpha_182(code, benchmark, end_date=None, fq=None):
    '''
    公式：
...
def alpha(code, benchmark, end_date=None, fq=None):
