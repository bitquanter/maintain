...
def get_pool(index,enddate=None):
    ...
    if index == 'all':
        return list(get_all_securities(['coin'],date=enddate).index)
    else:
        return get_index_coins(index,date=enddate)
...
def sign(x):
    ...
    Inputs:
        x: 一个Series, index为标的代码，values为需要判断正负号的数值

    Outputs:
        result: 一个Series, index为标的代码，values为+1或-1
    '''
    result = pd.Series([0]*len(x),index=x.index)
    for coin in x.index:
        result[coin] = math.copysign(1,x[coin])
...
def rank(series_need_rank,security_or_date):
    '''
    Inputs:
        series_need_rank: 一个需要rank的series，这个series的index为所有标的的代码或者日期
        security_or_date: 对应的需要rank的标的代码-在所有标的中rank这支标的对应值,或者对应的需要rank的date-在所有时间中rank这一天的值
...
def stddev(dataframe):
    ...
    Inputs:
        dataframe: 需要求标准差的dataframe（已经把长度切割好的）,index为时间,columns为标的池

    Outputs:
        标准差series,index为标的代码
    '''
    outputs = pd.Series([0.0]*len(dataframe.columns),index=dataframe.columns)
    length = len(dataframe.index)
    for coin in dataframe.columns:
        outputs[coin] = (sum([(dataframe[coin].iloc[j]-mean(dataframe[coin]))**2 for j in range(length)])/length)**(1/2)
        # temp = 0
        # for j in range(length):
        #     temp += (dataframe[coin].iloc[j]-mean(dataframe[coin]))**2
        # outputs[coin] = (temp/length)**(1/2)
    return outputs
...
def ts_argmax(dataframe):
    ...
    Inputs:
       dataframe : 需要进行求值的DataFrame，columns为标的代码，index为时间

    Outputs:
       一个Series，index为标的代码，values为每只标的这几天最大的值对应的序数，（第一天为1，有d天，最后一天为d，若最大值发生在第二天，
       则这只标的对应值为2）
    '''
    ...
    for coin in dataframe.columns:
        max_index, max_value = max(enumerate(dataframe[coin]), key=operator.itemgetter(1))
        ts_argmax_series[coin] = max_index+1
...
def ts_argmin(dataframe):
    ...
    Outputs:
       一个Series，index为标的代码，values为每只标的这几天最小的值对应的序数，（第一天为1，有d天，最后一天为d，若最大值发生在第二天，
       则这只标的对应值为2）
    '''
    ...
    for coin in dataframe.columns:
        min_index, min_value = min(enumerate(dataframe[coin]), key=operator.itemgetter(1))
        ts_argmin_seires[coin] = min_index+1
...
def adv(dataframe_money,d,y):
    ...
    Inputs:
       dataframe_money:columns为标的代码，index为时间，index的行数应该为d+(y-1)行
       y: value of adv{d} over the past y days

    Outputs:
       一个DataFrame，columns为标的代码，index为时间，从enddate开始之前的y天
    '''
...
def correlation_pool(dataframe1,dataframe2):
    ...
    Outputs:
       一个series，index为标的代码，values为对应的相关系数的数值

    '''
    output = pd.Series([0.0]*len(dataframe1.columns),index=dataframe1.columns)

    for coin in dataframe1.columns:
        output[coin] = correlation(dataframe1[coin],dataframe2[coin])
...
def covariance_pool(dataframe1,dataframe2):
    ...
    Outputs:
         一个series，index为标的代码，values为对应的covariance的数值
    '''
    output = pd.Series([0.0]*len(dataframe1.columns),index=dataframe1.columns)
    length = len(dataframe1.index)
    for coin in dataframe1.columns:
        left = sum([dataframe1[coin].iloc[i]*dataframe2[coin].iloc[i] for i in range(length)])/length
        right = (sum(dataframe1[coin])/length)*(sum(dataframe2[coin])/length)
        output[coin] = left-right
...
def ts_min(dataframe):
    ...
    Outputs:
       一个series，index为标的代码，values为min值
...
def decay_linear_pool(dataframe):
    ...
    Outputs:
       一个series，index为标的代码，values为decay_linear值
...
def delta(dataframe,d):
    ...
    Outputs:
       一个series，index为标的代码，values为delta值
...
def ts_rank(dataframe):
    ...
    output = pd.Series([0.0]*len(dataframe.columns),index=dataframe.columns)
    for coin in dataframe.columns:
        output[coin] = rank(dataframe[coin],dataframe.index[-1])
...
def alpha_001(enddate,index='all'):
    ...
    for coin in pool:
        ...
        for i in range(5):
            ...
            if returns_df[coin].iloc[-5+i]<0:

                signedpower_df[coin].loc[i] = signedpower(stddev(returns_df[coin].iloc[i:20+i].to_frame()),2).iloc[0]

            else:
                signedpower_df[coin].loc[i] = signedpower(close[coin].iloc[20+i],2)


#         print(signedpower_df[coin])
...
def alpha_007(enddate,index='all'):
    ...
    for coin in pool:
        ...
        if adv20[coin].iloc[0]<volume[coin].iloc[0]:
            result_series[coin] = -1*ts_rank_series[coin]*sign_series[coin]

        else:
            result_series[coin] = -1
...
def alpha_009(enddate,index='all'):
    ...
    for coin in pool:
        if ts_min_series[coin]>0:
            result_series[coin] = delta_df_01[coin].iloc[-1]
        elif ts_max_series[coin]<0:
            result_series[coin] = delta_df_01[coin].iloc[-1]
        else:
            result_series[coin] = -delta_df_01[coin].iloc[-1]
...
def alpha_010(enddate,index='all'):
    ...
    for coin in pool:
        ...
        if ts_min_series[coin]>0:
            series_willbe_ranked[coin] = delta_df_01[coin].iloc[-1]

        elif ts_max_series[coin]<0:
            series_willbe_ranked[coin] = delta_df_01[coin].iloc[-1]

        else:
            series_willbe_ranked[coin] = -1*delta_df_01[coin].iloc[-1]
...
def alpha_017(enddate,index='all'):
    ...
    for coin in pool:
        try:
            division[coin] = volume[coin]/adv20[coin]
        except:
            division[coin] = 0
            null_list.append(coin)
...
def alpha_021(enddate,index='all'):
    ...
    for coin in pool:
        try:
            division[coin] = volume[coin].iloc[-1]/adv20[coin].iloc[-1]
        except:
            division[coin] = 0

    for coin in pool:
        if mean_close_8[coin] + stddev_series[coin] <  mean_close_2[coin]:
            result[coin] = -1
        elif mean_close_2[coin]< mean_close_8[coin] + stddev_series[coin]:
            result[coin] = 1
        elif division[coin]>=1:
            result[coin] = -1
        else:
            result[coin] = -1
...
def alpha_023(enddate,index='all'):
    ...
    for coin in pool:
        if mean(high[coin]) < high[coin].iloc[-1]:
            result_series[coin] = -1*delta_series[coin]
        else:
            result_series[coin] = 0
...
def alpha_024(enddate,index='all'):
    ...
    result_series = pd.Series([0.0]*len(pool),index=pool)
    for coin in pool:
        if delta_mean_series[coin]/delay_series[coin]<=0.05:
            result_series[coin] = -1*(close[coin].iloc[-1]-ts_min_series[coin])
        else:
            result_series[coin] = -1*delta_series_02[coin]
...
def alpha_039(enddate,index='all'):
    ...
    for coin in pool:
        for i in range(len(volume.index)):
            if adv20[coin].iloc[i] == 0:
                decay_df[coin].iloc[i] = 0
            else:
                decay_df[coin].iloc[i] =volume[coin].iloc[i]/adv20[coin].iloc[i]
...
def alpha_043(enddate,index='all'):
    ...
    for coin in pool:
        for i in range(len(volume.index)):
            try:
                volume_adv_ratio[coin].iloc[i] = volume[coin].iloc[i]/adv20[coin].iloc[i]
            except:
                volume_adv_ratio[coin].iloc[i] = 0
...
def alpha_046(enddate,index='all'):
    ...
    #0.25 < (  delay(close, 20) - delay(close, 10)  ) / 10 - (delay(close, 10) - close) / 10
    for coin in pool:
        if (close[coin].iloc[0]-close[coin].iloc[-11])/10-(close[coin].iloc[-11]-close[coin].iloc[-1])/10>0.25:
            result[coin] = -1
        elif (close[coin].iloc[0]-close[coin].iloc[-11])/10-(close[coin].iloc[-11]-close[coin].iloc[-1])/10<0:
            result[coin] = 1
        else:
            result[coin] = -1*(close[coin].iloc[-1]-close[coin].iloc[-2])
...
def alpha_047(enddate,index='all'):
    ...
    for coin in pool:
        try:
            close_reciprocal[coin] = 1/close[coin].iloc[-1]
            adv20_reciprocal[coin] = 1/adv20[coin].iloc[0]
        except:
            close_reciprocal[coin] = np.nan
            adv20_reciprocal[coin] = np.nan
...
def alpha_049(enddate,index='all'):
    ...
    result = pd.Series([0.0]*len(pool),index=pool)
    for coin in pool:
        if (close[coin].iloc[0]-close[coin].iloc[-11])/10-(close[coin].iloc[-11]-close[coin].iloc[-1])/10<-0.1:
            result[coin] = 1
        else:
            result[coin] = -1*(close[coin].iloc[-1]-close[coin].iloc[-2])
...
def alpha_051(enddate,index='all'):
    ...
    result = pd.Series([0.0]*len(pool),index=pool)
    for coin in pool:
        if (close[coin].iloc[0]-close[coin].iloc[-11])/10-(close[coin].iloc[-11]-close[coin].iloc[-1])/10<-0.05:
            result[coin] = 1
        else:
            result[coin] = -1*(close[coin].iloc[-1]-close[coin].iloc[-2])
...
def alpha_056(enddate,index='all'):
    ...
    for coin in pool:
        try:
            cap_series[coin] = get_fundamentals(query(valuation.market_cap).filter(valuation.code==coin),date=enddate)['market_cap'].iloc[0]
        except:
            cap_series[coin] = np.nan
...
def alpha_061(enddate,index='all'):
    ...
    coin_series = pd.Series([0]*len(pool),index=pool)
    for coin in temp1_series.index:
        if temp1_series[coin]<temp2_series[coin]:
            coin_series[coin] = 1
        else:
            coin_series[coin] = -1
...
def alpha_062(enddate,index='all'):
    ...
    for coin in pool:
        if temp2_02[coin]<temp2_01[coin]:
            temp2_series[coin] = 1
        else:
            temp2_series[coin] = -1
...
def alpha_064(enddate,index='all'):
    ...
    for i in range(17):
        for coin in high.columns:
            sum_dataframe_17_01[coin].ix[i] = sum(openprice[coin].ix[i:i+13])*0.178404+sum(low[coin].ix[i:i+13])*0.821596
    ...
    for i in range(17):
        for coin in pool:
            sum_dataframe_17_02[coin].ix[i] = sum(adv120[coin].ix[i:i+13])
...
def alpha_065(enddate,index='all'):
    ...
    for i in range(6):
        for coin in pool:
            dataframe02[coin].ix[i] =sum(adv60[coin].ix[i:i+9])
...
def alpha_071(enddate,index='all'):
    ...
    result = pd.Series([0.0]*len(pool),index=pool)
    for coin in pool:
        result[coin] = max(temp1[coin],temp2[coin])
...
def alpha_073(enddate,index='all'):
    ...
    result = pd.Series([0.0]*len(pool),index=pool)
    for coin in pool:
        result[coin] = max(temp1[coin],temp2[coin])*(-1)
...
def alpha_077(enddate,index='all'):
    ...
    result = pd.Series([0.0]*len(pool),index=pool)
    for coin in pool:
        result[coin] = min(temp1[coin],temp2[coin])
...
def alpha_078(enddate,index='all'):
    ...
    result = pd.Series([0.0]*len(pool),index=pool)
    for coin in pool:
        result[coin] = temp1[coin]**temp2[coin]
...
def alpha_092(enddate,index='all'):
    ...
    for coin in pool:
        ...
        for i in range(33):
            if compare_01[coin].iloc[i]<compare_02[coin].iloc[i]:
                df_01[coin].loc[i] = 1
            else:
                df_01[coin].loc[i] = -1
